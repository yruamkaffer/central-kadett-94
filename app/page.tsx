...
export default function Home() {
  ...
  const [notice, setNotice] = useState("Dados salvos somente neste navegador");
  ...
  function announce(message: string) { setNotice(message); window.setTimeout(() => setNotice("Dados salvos somente neste navegador"), 3000); }
  ...
  return <div className="app-shell">
    ...
    <header className="topbar">
      ...
      <button className="garage-button" onClick={() => setGarage(true)}><span>▣</span> MODO GARAGEM</button>
    </header>
    ...
    <main id="conteudo" className="content" tabIndex={-1}>
      ...
      {view === "biblioteca" && <EmptyView eyebrow="ARQUIVO TÉCNICO" title="Biblioteca de referências" text="Guarde links, páginas de referência e anotações sem publicar documentos protegidos." action="Adicionar referência" />}
      {view === "historia" && <EmptyView eyebrow="MEMÓRIA DO VEÍCULO" title="História do Kadett" text="Uma linha do tempo para fatos verificados, relatos e momentos importantes deste carro." action="Criar primeiro registro" />}
...
function Cockpit({ state, openIssues, urgent, totalSpent, nextService, setView, setModal }: any) {
  return <>
    <PageHead eyebrow="VISÃO GERAL // VEÍCULO 01" title="Bom dia, motorista." text="Aqui está o estado registrado do seu Kadett agora." />
    <section className="vehicle-hero panel">
      <div className="vehicle-copy"><div className="hero-kicker">CHEVROLET <span>•</span> 1994</div><h2>KADETT <em>GLS</em></h2><p>1.8 EFI <span>•</span> COMBUSTÍVEL NÃO CONFIRMADO</p><div className="mileage"><small>QUILOMETRAGEM ATUAL</small><strong>{state.vehicle.currentMileage ? state.vehicle.currentMileage.toLocaleString("pt-BR") : "— — — — — —"}<b> KM</b></strong><button onClick={() => setModal("mileage")}>ATUALIZAR →</button></div></div>
    ...
    <section className="metric-grid">
      <button className="metric" onClick={() => setView("pendencias")}><span className="metric-icon amber">!</span><div><small>PENDÊNCIAS ABERTAS</small><strong>{openIssues.length.toString().padStart(2, "0")}</strong><p>{urgent.length ? `${urgent.length} exige atenção` : "Nenhuma urgência registrada"}</p></div><b>→</b></button>
      <button className="metric" onClick={() => setView("manutencao")}><span className="metric-icon cyan">⌁</span><div><small>PRÓXIMA MANUTENÇÃO</small><strong>{nextService?.next?.remainingKm != null ? `${nextService.next.remainingKm.toLocaleString("pt-BR")} km` : "Desconhecida"}</strong><p>{nextService ? nextService.name : "Cadastre intervalos confirmados"}</p></div><b>→</b></button>
    ...
    <section className="dashboard-grid">
      ...
      <div className="panel attention-panel"><div className="panel-title"><div><small>PRIORIDADE</small><h3>Atenção agora</h3></div><span>{urgent.length.toString().padStart(2, "0")}</span></div>{urgent.length ? <div className="attention-list">{urgent.slice(0, 3).map((issue: any) => <button key={issue.id} onClick={() => setView("pendencias")}><i className={issue.priority === "Crítica" ? "critical" : "high"}>{issue.priority[0]}</i><div><b>{issue.title}</b><small>{issue.subsystem} • {issue.status}</small></div><span>{formatBRL(issue.estimatedCost || 0)}</span></button>)}</div> : <div className="empty-mini"><b>Nenhuma urgência registrada</b><p>Isso não confirma que o veículo foi inspecionado.</p></div>}<button className="secondary full" onClick={() => setModal("issue")}>＋ REGISTRAR PENDÊNCIA</button></div>
    </section>
  </>;
}
...
function EmptyView({ eyebrow, title, text, action }: any) { return <><PageHead eyebrow={eyebrow} title={title} text={text} /><EmptyCard title="Módulo pronto para receber registros" text="A estrutura local está preparada. Este conteúdo será expandido na próxima fase." action={action} /></> }
